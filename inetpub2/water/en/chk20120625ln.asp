...
if request("submit")<>"" then
	   ...
	   sql = "select a.*,b.bal as xamt  from autopay a,loanrec b  where a.code='E1'  and a.memno=b.memno and a.lnnum=b.lnnum  "
	   rs.open sql,conn,1,1 
           do while not rs.eof
              amount =rs("xamt") + rs("bankin")
              response.write(rs("memno"))
                 response.write("---")
              response.write(amount)
               response.write("<br>")
              conn.execute("update loanrec set amount =  "&amount&" where lnnum = '"&rs("lnnum")&"' ")
           rs.movenext
           loop
           rs.close
           conn.execute("update loanrec set bal=amount where amount> 0  ")
	conn.committrans
