...
SQl = "select memno,memname,memcname  from memmaster where  "&styfield&" accode='"&memno&"'   and mstatus not in ('C','P','B' ) order by memno   "
...
<body leftMargin="10" topmargin="10" marginheight="0" marginwidth="0" media="print" >
<style type='text/css'>
...
rs1.close
       ...
       cut = 1
       if ylnnum<>"" then
          set ms = conn.execute("select *  from loan where  memno='"&xmemno&"'   and ldate<='"&yydate&"'   order by memno,ldate,code ") 
             cc = 0
             xx = 1
             qx = 0 
             MX=0
       ...
       if left(ms("code"),1) ="E" or ms("code")="0D" or ms("code")="D0"  or ms("code")="D1" OR ms("code")="DE" or ms("code")="NE" then
          lnnum(xx) = ms("lnnum")
               newln = 0
              xyear = year(ms("ldate"))
              xmon  = month(ms("ldate"))
     ...
     if (ms("code")="F3"  or ms("code")="F1" or ms("code")="F2" or ms("code")="F7" or ms("code")="F0"  )   then
 
       
           if ldate(xx-1) = ms("ldate") and xcode(xx-1) = "E"&right(ms("code"),1) then
             lniamt(xx-1) = ms("amount")
           end if
           if ldate(xx-1) <> ms("ldate") and (left(ms("code"),1) = "F" and ms("code")<>"F3" )  then
             lnnum(xx) = ms("lnnum")
             newln = 0
              xyear = year(ms("ldate"))
              xmon  = month(ms("ldate"))
