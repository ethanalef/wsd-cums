...
if request.form("action") <> "" then
	...
	Next
		...
		conn.begintrans
                ...
                select case mstatus
                       case "V","N","H","F","C","B","P","9","8"
                           if cashamt <>""  and cashamt <> 0 then 
                              if lnnum<>"" then
                                   amount = bal - cashamt  
                              conn.execute("insert into loan (memno,lnnum,code,ldate,amount) values ('"&memno&"','"&lnnum&"','"&ecode&"','"&xrepaydate&"',"&cashamt&") ")
			      conn.execute("update loanrec set bal=bal - "&cashamt&" where lnnum='"&lnnum&"' " )
                           ...
                           if intamt <> "" and intamt <> 0 then
                              if lnnum<>"" then      
                              conn.execute("insert into loan (memno,lnnum,code,ldate,amount) values ('"&memno&"','"&lnnum&"','"&fcode&"','"&xrepaydate&"',"&intamt&") ")
                              end if           
                           end if 
                       ...
                       case "A","1","2","3","M","T","0"                                                         
                           ...
                           if cashamt <>0 then
                              if pamt > 0 then
                                  ...
                                  if xx >= pamt then
                                     set rs = server.createobject("ADODB.Recordset") 
                                     sql = "select * from loan where lnnum='"&lnnum&"'  and code='DE'   and pflag = '1' "
    			            ...
    			            conn.execute("update loanrec set bal=bal - "&cashamt&" where lnnum='"&lnnum&"' " )
                                    ...
                                    conn.execute("insert into loan (memno,lnnum,code,ldate,amount,pflag,bal) values ('"&memno&"','"&lnnum&"','"&ecode&"','"&xrepaydate&"',"&cashamt&",0,"&xx&" ) ")                           
                                else
                                        amount = bal - cashamt
                                    conn.execute("update loanrec set bal=bal - "&cashamt&" where lnnum='"&lnnum&"' " )
                                    conn.execute("update loanrec set cleardate='"&xrepaydate&"' where lnnum='"&lnnum&"' and bal=0 " )
                                    ...
                                    conn.execute("insert into loan (memno,lnnum,code,ldate,amount,pflag,bal) values ('"&memno&"','"&lnnum&"','"&ecode&"','"&xrepaydate&"',"&cashamt&",1,"&cashamt&" ) ")                              
                                end if
                            '   if intamt = 0  and xx > 0 then
                          ...
                          if pint  >0  then
                              xint = 1
                                ...
                                if pint>0 then
                                     ...
                                     sql ="select * from loan where lnnum='"&lnnum&"'  and code='DF'   and pflag = 1 "
                                     rs.open sql, conn, 2, 2
                                     Do while not rs.eof  and cc = 0
                                        if xx - rs("bal") >= 0 then 
                                          rs("pflag")=0 
                                    ...
                                    conn.execute("insert into loan (memno,lnnum,code,ldate,amount,pflag,bal ) values ('"&memno&"','"&lnnum&"','"&fcode&"','"&xrepaydate&"',"&intamt&",0,"&xx&" ) ")                                      
                             
                           
                            else
                                 xint = 0                                  
                                  conn.execute("insert into loan (memno,lnnum,code,ldate,amount,pflag,bal ) values ('"&memno&"','"&lnnum&"','"&fcode&"','"&xrepaydate&"',"&intamt&",1,"&intamt&" ) ")                                      
                            end if
                        end if
                end select
                if saveamt <>0 then
                if saveamt >0 then
                                ...
                                if xx>=samt then
                                    ...
                                    rs.open sql, conn, 2, 2
                                     Do while not rs.eof  and cc = 0
                                        if xx - rs("bal") >= 0 then 
                                          conn.execute("update share set pdate='"&xrepaydate&"'  where memno='"&memno&"' and code='AI' and pflag= 1  and ldate ='"&rs("ldate")&"'  ")
                                          conn.execute("update share set pflag=0 where memno='"&memno&"' and code='AI' and pflag= 1 and ldate ='"&rs("ldate")&"'   ")
                                        else
                                          conn.execute("update share set pdate='"&xrepaydate&"'  where memno='"&memno&"' and code='AI' and pflag= 1 and ldate ='"&rs("ldate")&"'  ") 
                                          conn.execute("update share set bal= bal - "&xx&"   where memno='"&memno&"' and code='AI' and pflag= 1 and ldate ='"&rs("ldate")&"'   ")
                                           cc = 0
                                        end if
                                        xx = xx - rs("bal")
                                      rs.movenext
                                   ...
                                   conn.execute("insert into share (memno,code,ldate,amount,pflag,bal) values ('"&memno&"','"&acode&"','"&xrepaydate&"',"&saveamt&",0,"&xx&"  ) ")                                      
                             else
                                                                            
                                   conn.execute("insert into share (memno,code,ldate,amount,pflag,bal) values ('"&memno&"','"&acode&"','"&xrepaydate&"',"&saveamt&",1,"&saveamt&"  ) ")                                      
                              end if                                
...
<table border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
               <td width=30></td>
		<td class="b12" align="left">總結欠金額</td>
		<td width=50></td>
