...
sql = "select a.memno,a.lnnum,a.lndate,a.appamt,b.memname,b.memhkid from loanrec a,memmaster b where a.repaystat='N' and a.memno=b.memno  and (b.mstatus='B' or b.mstatus='P'  or b.mstatus='F')  "& sql_filter & " order by lnnum desc"
...
<form name="form1" method="post" action="scloan.asp">
<div><center><font size="3">現金清數(本金)</font></center></div>
...
%>
  <tr bgcolor="#FFFFFF">
	<td><a href="scloandetail.asp?lnnum=<%=rs("lnnum")%>"><font size="2"><%=rs("lnnum")%></font></a></td>
...
<%if session("cpageno")>1 then%>
    <a href="scloan.asp?upage=upage<font size="2">上一頁</font></a>
<%end if%>
<%if session("cpageno")< pagecount then%>
<a href="scloan.asp?npage=npage<font size="2">下一頁</font></a>
<%end if %>
<%end if%>
</center>
</body>
