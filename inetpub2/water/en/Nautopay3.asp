...
if request("submit")<>"" then
  ...
  sql = "select a.memno,a.mstatus,a.monthsave,a.monthssave,a.tpayamt,a.bnklmt,b.lnnum,b.appamt,b.lndate,b.monthrepay,b.bal,b.delyflag,b.chkmon,b.months,b.chequeamt,isnull(b.lnflag,'N') as lnflag  ,b.loantype,b.oldlnnum    from memmaster a,loanrec b "&_
    ...
    do while not rs.eof
        ...
        if lnnum<>"" then
          if (mstatus = "M" or mstatus = "T") and monthssave > 0 then
            pamt1 = monthssave
          elseif rs("bal") > rs("monthrepay")  then
            pamt1 = rs("monthrepay")
          else
            pamt1 = rs("bal")
          end if
            ...
            yy  = xyy
            mn  = xmn
            mdd = day(dateserial(xyy,xmn+1,0)) 'get the last day of the month
            ...
            case "0"
                    ...
                    if ttlsave > 0 then
                       ...
                       case "3","8"
                        ...
                        case "M"
                          if tpayamt > ttlpint then
                            ...
                            if difamt < ttlpamt then
                              ...
                              ' 庫房還息 (F2)
                              conn.execute("insert into autopay (memno,adate,lnnum,code,bankin,curamt,updamt,status,flag,pdate,deleted,pflag ) values ('"&memno&"','"&mdate&"','"&lnnum&"','F2',"&tpayamt&","&pint1&","&pint2&",'M','N','"&pdate&"',0,0 ) ")
                              difamt = ttlpint - tpayamt
                              ' 銀行還息 (F1)
                        ...
                        case "T"
                          if tpayamt > (ttlpint+ttlpamt) then
                            saveamt = tpayamt - ttlpint - ttlpamt
                            ' 庫房還息 (F2)
