...
do while not rs.eof
           if memno<>rs("memno")  then   
				'if memno=20 then sttlamt=4629.97 end if			
               if sttlamt > 0 then
                   ttlcnt = ttlcnt + 1
                   if rs("flag") = "F" then
                      ttlxcnt = ttlxcnt + 1
...
%>
	...
	<tr>
		<td>合共</td>
		<td></td>
	        <td align="right"><%=formatNumber(Aint,2)%></td>
                <td align="right"><%=formatNumber(Apri,2)%></td>
                <td align="right"><%=formatNumber(Asha,2)%></td>
		...
		<td align="right"><%=formatNumber(Aamt,2)%></td>
	</tr>
</table>
</center>
<table border="0" cellpadding="0" cellspacing="0">
