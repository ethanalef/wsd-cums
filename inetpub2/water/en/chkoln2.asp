...
if request("submit")<>"" then
        ...
        conn.begintrans
	sql =  "select * from loanrec where repaystat = 'L'   "
        rs.open sql, conn, 1, 1          
        do while not rs.eof

           if rs("lnflag")="Y" then
              set rs1 = conn.execute("select bal from loanrec where lnnum='"&rs("oldlnnum")&"' ")
              if not rs1.eof then
                 xbal = rs1(0)+rs("chequeamt")
              end if
              rs1.close
           else
              xbal = rs("appamt")
        ...
        loop
        rs.close
        connn.execute("update loanrec set repaystat = 'N' where repaystat ='L' ")
	conn.committrans
