...
for i = 0 to 12
	...
	dvdate(i)=dateserial(xyear, 7+i, 1)
next

sql = "select a.memno, a.memname, a.memcname, a.B1, a.B2, a.B1relation, a.B2relation from memmaster a "&_
		"where exists ("&_
			"select memno from share as b "&_
				"where ldate < '"&nndate&"' and ldate >= '"&mmdate&"' and a.memNo = b.memno and "&_
					styfield & "mstatus not in ('C','P','B','I' ) and wdate is null) "&_
		"order by a.memno"
...
' balance propagation
	...
	for i = 1 to 12
		...
		'if rs("memno")=>4934 and right(round(divd(i)),2)="36" and divd(i-1)=rs.bof then
		'	divd(i)=divd(i)-36
		'end if
	next
...
<font face="arial, helvetica, sans-serif" size="3" ><b>　　　　　  　　加入閣下的電話簿內即可。同時亦可隨時登入本社網址 wsdscu.com 瀏覽所有通告及最新消息。</b></font><br>
