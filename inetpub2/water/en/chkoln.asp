...
if request("submit")<>"" then
        ...
        conn.begintrans
	sql =  "select * from loanrec where  lndate >='"&xdate&"' "
        rs.open sql, conn, 1, 1
        do while not rs.eof
           if rs("lnflag") = "Y"  then
              oldln = rs("oldlnnum")
              conn.execute("update loanrec set repaystat = 'N' where lnnum = '"&rs("oldlnnum")&"' ")
              conn.execute("update loanrec set cleardate =  null where lnnum = '"&rs("oldlnnum")&"' ")
              conn.execute("delete loan where lnnum = '"&rs("lnnum")&"' ")
              conn.execute("delete loan where lnnum = '"&rs("oldlnnum")&"' and code='D0'  ")
              conn.execute("update loanrec set  repaystat ='L' where lnnum = '"&rs("lnnum")&"' ")
        ...
        loop
        rs.close   
        conn.execute("delete loan where (code='FI') or (code='MF' and amount=0)   ")
