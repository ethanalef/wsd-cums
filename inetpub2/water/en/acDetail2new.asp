...
if request.form("Search")<>"" then
	...
	Next
       ttlbal = 0
        id = memno
       sql ="select a.* ,b.dttl,b.cttl   from memmaster a    "&_
                           ...
                           "  then amount else 0 end ) as cttl from share  where memno='"&memno&"'  group by memno ) b on a.memno=b.memno  where a.memno='"&memno&"' " 
         ...
         if isdate(membdaay) then 
           ...
           md = day(dateserial(yy,mm+1,1-1))
             if bal > monthrepay  then 
                ...
                pint2 = 0       
                pint1  = 0 
		set rs1  = conn.execute("select   isnull(sum(case when code='DE' then  amount else 0 end ),0) , "&_
                                 " isnull(sum( case when code='DF' then amount else 0 end ) ,0)  from loan where memno='"& memno & "'  and pflag= 1 ")				
                if not rs1.eof then
                   pamt2 = rs1(0)
                   pint2 = rs1(1)   
                            ...
                            if lnflag="Y" then
                              if appamt=bal then                                
                               ...
                               a2 = 0
                                  ...
                                  if ms( appamt -bal ) <> monthrepay then
                                     a2 = round(chequeamt*.01*(xmD-dd+1)/xmD,2)
                                     xintamt = a2
                                  end if
              ...
              sql ="select a.* ,b.ttl from guarantor a   "&_
                   " right join ( select memno ,(sum( case when left(code,1)  in  ( '0','A','C' ) and code<>'AI' and code<>'CH' "&_
                   " then amount else 0 end ) -  sum( case when left(code,1)  in  ( 'G','H','B' ) and code<>'MF' "&_
                   " then amount else 0 end )) as ttl from share  group by memno ) b on a.guarantorID=b.memno "&_
                   " where a.lnnum='"&lnnum&"'  "
          ...
          sql = "select a.*,b.memname,b.memcname  ,c.bal  from guarantor a    "&_
                 " inner join memmaster b on a.memno = b.memno  "&_
                 " right join ( select lnnum,bal from loanrec where repaystat='N' ) c "&_
                 "  on a.lnnum = c.lnnum  where a.guarantorid ='"&memno&"'  "
