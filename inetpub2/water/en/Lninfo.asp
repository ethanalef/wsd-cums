...
<%
        conn.begintrans'
  '      conn.execute("drop table #tempa")
	conn.execute("CREATE TABLE #tempa ( [memno] [int]  ,[tdate] [smalldatetime]  ,[scode] [char] (2)  ,[amount] [money]  ,[lnnum] [char] (10)  ,[intamt] [money] ,[cashamt] [money] ,[lncode] [char] (2) )  ")


	conn.committrans			
	SQl = "select * from share where memno='2458'   "
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.open sql, conn,1,1
	SQl1 = "select * from #tempa  where 0=1"
	...
	rs1.open sql1, conn,2,2
       ...
       rs1.close
        xx = 0 
	SQl = "select * from loan where memno='2458' "
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.open sql, conn,1,1

        do while not rs.eof
        
	SQl1 = "select * from #tempa where tdate = '"&rs("ldate")&"'  "
	Set rs1 = Server.CreateObject("ADODB.Recordset")
	rs1.open sql1, conn,2,2
        if not rs1.eof then
           if xx >= 1 then
           ...
           select case rs("code")
                  case "E1","EI" 
  		     rs1("lnnum")= rs1("lnnum")                  
                     rs1("cashamt")=rs("amount")
                     rs1("lncode")=rs("code")
                     
                  case "F1","FI" 
                      rs1("intamt")=rs("amount")
                      xx = xx+1
           end select
           rs1.update
           rs1.close
...
SQl = "select a.*,b.name from #tempa a,memmaster b where a.memno=b.memNo  order by a.memno"
Set rs = Server.CreateObject("ADODB.Recordset")
rs.open sql, conn
...
elseif request.form("output")="text" then
	...
	do while not rs.eof
		...
		objFile.WriteLine ""
 	       if rs("trefno") = "A1" then a1ttl =  a1ttl + rs("amount") end if
       	       if rs("trefno") = "AI" then aIttl =  aIttl + rs("amount") end if
       	       if rs("trefno") = "E1" then E1ttl =  E1ttl + rs("amount") end if
      	       if rs("trefno") = "EI" then a1ttl =  EIttl + rs("amount") end if
               if rs("trefno") = "F1" then F1ttl =  F1ttl + rs("amount") end if
