...
if key<>"" then
   if left(key,1)>="0" and left(key,1) <="9" then
	stylefield = "and b.lnnum like '" & key & "%'"
       if len(key) > 4 then
       else
...
SQL = "select a.memNo,a.memname,a.mstatus,b.lnnum from memmaster a,loanrec b where a.memNo=b.memno and repaystat='N'   "& stylefield 
...
<!--
	window.opener.document.form1.memNo.value='<% =rs("memNo") %>'
        window.opener.document.form1.mstatus.value='<% =rs("mstatus") %>'
        window.opener.document.form1.id.value='<% =rs("memNo") %>'
	window.opener.document.form1.memName.value='<% =rs("memName") %>'
        window.opener.document.form1.lnnum.value='<%=rs("lnnum")%>'
        window.opener.document.form1.xlnnum.value='<%=rs("lnnum")%>'
...
<script language="JavaScript">
<!--
function PostCode(memNo,memName,mstatus,lnnum)
{
	window.opener.document.form1.memNo.value=memNo;
        window.opener.document.form1.mstatus.value=mstatus;
        window.opener.document.form1.id.value=memNo;
	window.opener.document.form1.memName.value=memName;
	window.opener.document.form1.lnnum.value=lnnum;
	window.opener.document.form1.xlnnum.value=lnnum;
        window.opener.document.form1.Search.click()       
