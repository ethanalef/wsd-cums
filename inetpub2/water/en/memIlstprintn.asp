...
<table border="0" cellpadding="0" cellspacing="0">
	...
	<td width=70 align="center"><font size="3"  face="標楷體" >中文姓名</font></td>	
       <td width=70 align="center"><font size="3"  face="標楷體" >職業</font></td>	
...
<% 
   ...
   do while not rs.eof 
      ...
      if xmemno<> rs("memno")   then
         set ms=conn.execute("select memname,memcname,membday,memhkid,memGender,membday,memGrade,mstatus  from memmaster where memno='"&xmemno&"' ")
         if not ms.eof then
             memname = ms("memname")    
...
%>
     <tr>
          ...
          <td width="80"><font size="3" face="標楷體"  ><%=xmemgrade%></font></td>
...
<%
    ...
    loop
    rs.close
         set ms=conn.execute("select memname,memcname,membday,memhkid,memGender,membday,mstatus  from memmaster where memno='"&ymemno&"' ")
         if not ms.eof then
             memname = ms("memname")    
             memcname=ms("memcname")
             xmstatus=  ms("mstatus")
