...
if request.form("action") <> "" then
	...
	if msg="" then
		...
		conn.begintrans  
                ...
                set rs=conn.execute("select amount from loan where lnnum='"&newlnnum&"' and code='D1'  ")
                newappamt = rs(0) 
                rs.close
                set rs=nothing
                ...
                conn.execute("insert into loan (memno,lnnum,code,ldate,amount) values ( '"&memno&"','"&lnnum&"','D0','"&xcldate&"',"&bal&") ")
	...
	if xlnnum <> "" then
		...
		sql = "select * from loanrec where memno = '"&xmemno&"' and repaystat='N' and lndate < '"& xlndate &"'  "
		...
		if rs.eof then
         ...
         newlnnum = xlnnum
         cleardate =  right("0"&day(xlndate),2)&"/"&right("0"&month(xlndate),2)&"/"&(year(xlndate))     	
