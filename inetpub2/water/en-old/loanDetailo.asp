...
if request.form("Search")<> "" then
    ...
    SQL = "select memNo,memName,memGrade,employCond,firstAppointDate,memBday,mstatus from memMaster where memno='"&memno&"' "
    rs.open sql,conn,2
    if not rs.eof then 
...
if request.form("action") <> "" then
	...
	if uid = "" then
		...
		rs.addnew
		addUserLog "Add Loan Application ID "&id&" Member No. "&memno
	else
		addUserLog "Modify Loan Application ID"&uid&" Member No. "&memno
	end if
	...
	rs("memGrade")=memGrade               
       ...
       if guarantor3Salary<>"" then rs("guarantor3Salary")=cdbl(guarantor3Salary) else rs("guarantor3Salary")=null end if
        ...
        rs("bnklnn")=bnklnn
        rs("bnklny")= bnklny 	        
        if bnklnamt<>"" then rs("bnklnamt") = bnklnamt else rs("bnklnamt") = 0 end if
...
%>
	...
	<tr height="25" valign="bottom">
		<td class="b8">申請人有否向其他銀行或財務機構申請借貸服務？</td>
        </tr>
        <tr>
            <td width="150" class="n8"><input type="checkbox" name="bnklnn"   value="<% =bnklnn %>"<%if bnklnn="Y" then response.write " checked" end if%> onclick="if (this.checked){clearbnk(1)}">無 </td>
	    <td width="100" class="n8"><input type="checkbox" name="bnklny" 　value="<% =bnklny %>"<%if bnklny="Y" then response.write " checked" end if%> onclick="if (this.checked){clearbnk(2)}">有 </td>
         </tr>
	<tr height="25" valign="bottom">
	<td class="b8">每月總供款額：港幣  <input type="text" name="bnklnamt" value="<%=bnklnamt%>" size="15" maxlength="15"></td>
        </tr>
</table>
...
<table border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		<td width="100" class="b8" align="right">貸款委員批核</td>
		<td width=10></td>
		<td width="100">
...
<%				lb.movenext
	...
	<tr>
		<td colspan="9" align="right" valign="middle">
			<%if session("userLevel")<>1 orsession("userLevel")<>2 or session("userLevel")<>6 then%>
