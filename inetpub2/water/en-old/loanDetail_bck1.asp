...
if request.form("Search")<> "" then
    ...
    if not rs.eof then 
        ...
        if dd < 13 then     
            ...
            IF xlndate <> "" THEN
                xlndate = CINT(right(xlndate,4)) & "/" & mid(xlndate,4,2)& "/" & CINT(left(xlndate,2))
            END IF
            
            SQL = "select * from loan where memno ='"&memno&"' and ldate>='"&xlndate&"' and ldate <='"&yydate&"' order by lnnum,ldate,right(code,1),left(code,1)"
            rs.open sql,conn,2   
            do while not rs.eof
...
if request.form("action") <> "" then        
    ...
    select case firstApproval
        ...
        if age<>"" then rs("age")=age else rs("age")=null end if 
	    if memGrade <> "" then rs("memGrade") = memGrade else rs("memGrade") = NULL end if
        ...
        conn.execute("update memaster set remark = '"&remark&"'  where memno='"&memno&"' ")
...
function validating(){
	...
	var x=document.getElementById("firstApproval").selectedIndex
	if (x==0){
	    reqField=reqField+", 批核申請";
	    if (!placeFocus)
			placeFocus=formObj.firstApproval;
	}

	if (formObj.firstApprovalDate.value==""){
		reqField=reqField+", 貸委會批核日期";
		if (!placeFocus)
			placeFocus=formObj.firstApprovalDate;
	}
	
    var x=document.getElementById("secondApproval").selectedIndex
    if (x==0){
		reqField=reqField+", 董事會批核";
		if (!placeFocus)
			placeFocus=formObj.secondApprovalDate;
	}
 
	if (formObj.secondApprovalDate.value==""){
