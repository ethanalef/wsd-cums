...
END IF
chk="Approved"
sql = "select a.memno,a.lnnum,a.lndate,a.appamt,b.memname,b.memhkid from loanrec a,memmaster b where a.repaystat='N' and a.memno=b.memno  "& sql_filter & " order by lnnum desc"
...
<form name="form1" method="post" action="shwdloan.asp">
<div><center><font size="3">股金清數</font></center></div>
...
%>
  ...
  <tr bgcolor="#FFFFFF">
	<td><a href="shwdlndetail.asp?lnnum=<%=rs("lnnum")%>"><font size="2"><%=rs("lnnum")%></font></a></td>
...
<%if session("cpageno")>1 then%>
    <a href="shwdloan.asp?upage=upage<font size="2">上一頁</font></a>
<%end if%>
<%if session("cpageno")< pagecount then%>
<a href="shwdloan.asp?npage=npage<font size="2">下一頁</font></a>
<%end if %>
<%end if%>
</center>
</body>
