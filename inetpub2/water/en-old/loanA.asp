...
if date1 <> "" then
    cdate1 = right(date1, 4) & "/" & mid(date1, 4, 2) & "/" & Left(Date1, 2)
      if  sql_filter <>""  then  
	  sql_filter = sql_filter & " and  appDate >= '"&cdate1&"'"
      else
          sql_filter = sql_filter & " appDate >= '"&cdate1&"'"
      end if
end if
if date2 <> "" then
    cdate2 = right(date2, 4) & "/" & mid(date2, 4, 2) & "/" & Left(Date2, 2)
     if  sql_filter <>""  then  
	sql_filter = sql_filter & " and  appDate <= '"&cdate2&"'  "
     else
	sql_filter = sql_filter & " appDate <= '"&cdate2&"'  "
     end if 
end if
...
'	response.redirect "loanDetaila.asp"
'end if
...
<form name="form1" method="post" action="loana.asp">
...
<%if recordcount>pagesize then navigator("loana.asp?uid="&uid&"&date1="&date1&"&date2="&date2&"&memNo="&memNo&"&memName="&memName&"&Approval="&Approval) end if%>
...
<%if session("userLevel")<>5 then%>
		<td bgcolor="#FFFFFF"><a href="loanDetaila.asp"><font size="2">新增</font></a></td>
<%end if%>
	</tr>
<%
...
%>
  <tr bgcolor="#FFFFFF">
	<td><a href="loanDetaila.asp?uid=<%=rs("uid")%>"><font size="2"><%=rs("uid")%></font></a></td>
