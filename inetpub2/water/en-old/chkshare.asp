...
if request("submit")<>"" then
        ...
        conn.begintrans
	sdate1 = dateserial(2008,12,1)
        sdate2 = dateserial(2009,1,31)
	sql = "select * from memmaster where wdate>='"&sdate1&"'  and wdate<='"&sdate2&"' "
	
	rs.open sql, conn, 1, 1       
        do while not rs.eof 
              xmemno = rs("memno")
              xbal = 0
           sqlstr = "select *  from share where memno ='"&xmemno&"' "
           ...
           do while not rs1.eof
              ...
              if xbal > 0 then
                 ...
                 conn.execute("update memmaster set wdate =null where memno='"&xmemno&"' ")
   ...
   response.write("total count : "&cnt)
	conn.committrans
