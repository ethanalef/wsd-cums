...
if request("from") = Request.ServerVariables("script_name") then
	...
	if request.form("submit")<>"" then
		...
		for idx = 1 to bdNum
			...
			if request.form("bd1c"&idx) = "" then
				sql = "select top 1 * from meetingNotes2 order by uid desc"
			else
				sql = "select * from meetingNotes2 where uid="&request.form("bd1c"&idx)
			end if
		...
		next
		if archive="" then
			conn.execute("delete from meetingNotes2 where rpId="&loan)
		else
			conn.execute("delete from meetingNotes2 where rpId="&loan&" and uid not in ("&left(archive,len(archive)-1)&")")
		end if
		conn.committrans
	...
	SQL = "select * from meetingNotes2 where rpId=" & loan
	set rs = server.createobject("ADODB.Recordset")
	rs.open sql, conn
	if rs.eof then
		bdNum = 0
