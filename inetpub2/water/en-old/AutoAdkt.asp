...
if request("del")<>"" then
  ...
  if status="否" then
     ...
     conn.execute("update autopay set flag='F'  where memno='"&smemno&"' and right(code,1)='1'  " )
  else
     conn.execute("update autopay set flag=''  where memno='"&smemno&"'  and right(code,1)='1'  " )
  end if   
  memno = smemno
...
sql = "select a.*,b.memname,b.memhkid from autopay a,memmaster b where  a.memno=b.memno and (right(a.code,1)='1' ) "& sql_filter & " order by a.memno "
...
<br>
<div><center><font size="3">銀行脫期建立</font></center></div>
<center>
<%if msg<>"" then response.write "<font color=red>"&msg&"</font>" end if%>
<form name="form1" method="post" action="AutoAdkt.asp">
...
<%if recordcount>pagesize then navigator("AutoAdkt.asp?lnnum="&lnnum&"&memhkid="&memhkid&"&memNo="&memNo&"&memName="&memName&"&sflag="&sflag) end if%>
