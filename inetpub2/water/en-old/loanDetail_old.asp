   ...
   SQL = "select amount,CODE  from share where memno='"&memno&"' "
    ...
    do while  not rs.eof 
       select case rs("code")
              ...
              case "B0","B1","B3","G0","G1","G3","H0","H1","H3","MF"
                  savettl = savettl - rs(0)
       end select
    rs.movenext
    loop 
    ...
    if dd < 13 and xloantype="N" then     
       ...
       if int(yy/4)=yy/4 and int(yy/100) and yy/100 and xmm= 2 then
          xdd = 29
       end if
       if xmm<10 then
          xmm = "0"&xmm
       ...
       SQLstr = "select *,convert(char(10),ldate,102) as xldate    from loan where memno='"&memno&"' and lnnum='"&xlnnum&"'    order by lnnum,ldate,right(code,1),left(code,1)  "
