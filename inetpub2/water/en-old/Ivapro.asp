...
if request.form("Search") <> "" then 
   ...
   if msg=""  then
    ...
    set rs=conn.execute("select * from share  where memno='"&memno&"'  and ldate<='"&xxdate&"'  order by memno,ldate,code   ")
     if not rs.eof then
        do while  not rs.eof 
           select case rs("code")
     ...
     amount = 0
     nmonth = 0
     set rs=conn.execute("select a.* ,b.appamt  from loan a,loanrec b where a.memno='"&memno&"' and a.memno=b.memno and a.lnnum=b.lnnum and b.repaystat='N' and a.ldate<='"&xxdate&"'  order by a.memno,a.ldate,a.code    ")
     if not rs.eof then
     do while not rs.eof 
...
<div><center><font size="3">IVA操作建立</font></center></div>
<center>
