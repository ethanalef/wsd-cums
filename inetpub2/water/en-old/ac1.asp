...
if searchkey<>"" and rs.recordcount=1 then
	response.redirect "acDetail2.asp?id="&rs("memNo")
end if
...
<br>
<center>
<form name="form1" method="post" action="ac1.asp">
...
%>
  <tr bgcolor="#FFFFFF">
	<td><a href="acDetail2.asp?id=<%=rs("memNo")%>"><font size="2"><%=rs("memNo")%></font></a></td>
...
<%if session("cpageno")>1 then%>
    <a href="ac1.asp?upage=upage<font size="2">上一頁</font></a>
<%end if%>
<%if session("cpageno")< pagecount then%>
<a href="ac1.asp?npage=npage<font size="2">下一頁</font></a>
<%end if %>
<%end if%>
</center>
</body>
