...
if request.form("Search")<> ""  then
   ...
   SQL = "select amount,CODE  from share where memno='"&memno&"' "
    ...
    do while  not rs.eof 
       select case rs("code")
              case "0A" ,"A1","A2","A3","C0","C1","C3","A0","A7" ,"A4" ,"C5"
                   savettl = savettl + rs(0)
              case "B0","B1","B3","G0","G1","G3","H0","H1","H3","MF","BE","BF"
                  savettl = savettl - rs(0)
       end select
    rs.movenext
    loop 
    ...
    if not rs.eof then  
      ...
      xlnflag = lnflag
       ...
       xdd = CINT(left(apcutdate,2))
       xmm = CINT(mid(apcutdate,4,2))
       yy = CINT( right(apcutdate,4))
        
 
       yydate = dateserial(yy,xmm,xdd)

      
       SQLstr = "select *,convert(char(10),ldate,102) as xldate    from loan where  lnnum='"&xlnnum&"'  and ldate >='"&xlndate&"'  order by lnnum,ldate,code  "
         ...
         select  case rs1("code")
                 ...
                 case "D9","0D","D8"
                       ...
                       do while not rs1.eof   
                           xdate = year(rs1("ldate"))&"."&right("0"&month(rs1("ldate")),2)&"."&right("0"&day(rs1("ldate")),2)           
                          if  rs1("ldate") <=yydate then
                           select case rs1("code")
                                 case "0D"
                                       lnbal = rs1("amount")
                                  case "D9"
...
if request.form("action") <> "" then
 ...
 rs("memGrade")=memGrade               
	...
	rs.update
        if xremark <> remark then        
        conn.execute("update memmaster set remark = '"&remark&"'  wehre memno='"&memno&"' ")
...
session("username") = username
	...
	if uid <> "" then
		...
		chequeDate=right("0"&day(date()),2)&"/"&right("0"&month(date()),2)&"/"&year(date())
                xdate = dateserial(year(date()),month(date())-1,1-1)
                apcutDate=right("0"&day(xdate),2)&"/"&right("0"&month(xdate),2)&"/"&year(xdate)
		interest=-1
		loanPlanID=0
		SpecialPlanID=0
    bnklnn ="N"
...
function calculation(){
	...
	if (formObj.loanAmt.value!=""&&formObj.loanAmt.value!=0&&formObj.installment.value!=""&&formObj.installment.value!=0&&formObj.chequeDate.value!=""){
		loanAmt=parseFloat(formObj.loanAmt.value)
...
function chkdate(){
        ...
        Y = 0;
        D = 0;
        cDate=document.form1.appDate.value
		strD = cDate.substr(0,2) * 1;
		strM = cDate.substr(3,2) * 1;
		strY = cDate.substr(6,4) * 1;       
      if ( strD >= 13) {
           M = strM -1;
      }else{
           M = strM -2;
      ...
      xday = "312931303130313130313031"
      pos = (M-1)*2;
      
      d = xday.substr(pos,2)*1;
      if (!valDate(M, d, Y)){
         d = 28;
       }
      if (M < 10){
       ...
       document.form1.apcutDate.value = d+"/"+M+"/"+Y ;
}
...
<table border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td width="300" valign="top">
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="b8" align="right">申請日期</td>
					<td width=10></td>
					<td>
						<input type="text" name="appDate" value="<%=appDate%>" size="10" maxlength="10" onblur="if(!formatDate(this)){this.value=''};chkdate();form1.firstApprovalDate.value=this.value;">
						 (dd/mm/yy)
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td width=10></td>
					<td>
						<input type="text" name="apcutDate" value="<%=apcutDate%>" size="10" maxlength="10" onblur="if(!formatDate(this)){this.value=''};">
						 (dd/mm/yy)
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td width=10></td>
					<td><input type="text" name="loanAmt" value="<%=loanAmt%>" size="17" maxlength="17" onblur="if(!formatNum(this)){this.value=''};"></td>
				</tr>
				<tr>
...
%>
	<tr height="25" valign="bottom">
		...
		<td colspan="3"><input type="text" name="otherReason1" value="<%=otherReason1%>" size="50" maxlength="50" ></td>
	</tr>
<%
xx = 1
...
%>
	<tr height="25" valign="bottom">
		...
		<td colspan="3"><input type="text" name="otherReason2" value="<%=otherReason2%>" size="50" maxlength="15" ></td>
        </tr>  
<%
if not rs.eof then
%>
...
<%
	do while not rs.eof
		...
		<td width="150" class="n8"><input type="checkbox" name="TS" value="<% =rs("uid") %>"<%if rs("theCheckPoint")<>0 then response.write " checked" end if%> onclick="if (this.checked){clearOthers(<% =rs("uid") %>)}"> <%=rs("reasonName")%>
		</td>
<%
		if locate = 3 then
			response.write "</tr>"
...
<%				lb.movenext
	...
	<tr>
		<td colspan="9" align="right" valign="middle">
			<%if session("userLevel")<>1 or session("userLevel")<>2 or session("userLevel") <>6 then%>
