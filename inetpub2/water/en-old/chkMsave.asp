...
if request("submit")<>"" then
	...
	rs.open sql, conn, 1, 1       
        do while not rs.eof 
           if rs("dtjoint")<>"" then
           conn.execute("update memmaster set memdate='"&rs("dtjoint")&"'  where memno='"&rs("acct")&"' ") 
           else
              conn.execute("update memmaster set memdate=null  where memno='"&rs("acct")&"' ")  
           end if
            if rs("dtclose")<>"" then
               conn.execute("update memmaster set wdate='"&rs("dtclose")&"'  where memno='"&rs("acct")&"' ")                  
             else
            conn.execute("update memmaster set wdate=null  where memno='"&rs("acct")&"' ")                  
             end if
        rs.movenext
	loop
	rs.close
