...
<script language="JavaScript">
<!--
function PostCode(memNo,memName,memGrade,employCond,firstAppointDate,memBday,savettl)
{
	...
	window.opener.document.form1.savettl.value=savettl;
	window.opener.document.all.tags( "td" )['savettl'].innerHTML=savettl;
...
<%
	do while not rs.eof and rowcount < rs.pagesize
		rowcount = rowcount + 1
sql1="select sum(amount) from share where memno='"&rs("memno")&"' "
...
%>
			<tr>
				<td bgcolor="#ffffff"><a href="JavaScript:PostCode('<%=rs(0)%>','<%=rs(1)%>','<%=rs(2)%>','<%=rs(3)%>','<%=right("0"&day(rs(4)),2)&"/"&right("0"&month(rs(4)),2)&"/"&year(rs(4))%>','<%=right("0"&day(rs(5)),2)&"/"&right("0"&month(rs(5)),2)&"/"&year(rs(5))%>','<%=savettl%>')"><% =rs("memNo") %></a></td>
				<td bgcolor="#ffffff"><a href="JavaScript:PostCode('<%=rs(0)%>','<%=rs(1)%>','<%=rs(2)%>','<%=rs(3)%>','<%=right("0"&day(rs(4)),2)&"/"&right("0"&month(rs(4)),2)&"/"&year(rs(4))%>','<%=right("0"&day(rs(5)),2)&"/"&right("0"&month(rs(5)),2)&"/"&year(rs(5))%>','<%=savettl%>')"><% =rs("memName") %></a></td>
			</tr>
<%
		rs.movenext
	loop
