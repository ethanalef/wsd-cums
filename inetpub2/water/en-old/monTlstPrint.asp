...
xidx = "#temp"&idx
  ...
  conn.begintrans
              ...
              conn.execute( "insert into "&xidx&"  (memno,ldate,code,amount ) select memno,ldate,code,amount from share where (right(code,1)='2' ) and  year(ldate) ='"&yy&"' and month(ldate) ='"&mm&"' order by memno,ldate,code "  )
              conn.execute( "insert into "&xidx&"  (memno,lnnum,ldate,code,amount ) select memno,lnnum,ldate,code,amount from loan  where (right(code,1)='2' ) and year(ldate)='"&yy&"' and month(ldate)='"&mm&"'  order by memno,ldate,code  " )
...
if rs.eof then 
   ...
   response.redirect "monTtlst.asp"
end if
...
<table border="0" cellpadding="0" cellspacing="0" >
	<tr height="30" valign="top" align="center"><td colspan="15"><font size="4"  face="標楷體" >水務署員工儲蓄互助社<br>庫房帳細項明細表<br><font size="2"  face="標楷體" >日期 : <%=mndate%></font></font></td></tr>
