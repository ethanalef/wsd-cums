...
<%if xlnnum<>"" then%>      
	...
	for i = 1 to 12
				...
				if rs("memno")=3674 then	divd(i)=divd(i)+26 	end if				
				if  divd(i) <>"" then
					'***********************************************************			
					...
					if rs("memno")=4804 and sldate="01/07/2019"  then 	divd(i)= 609.25  end if
					if rs("memno")=4804 and sldate="01/08/2019"  then	divd(i)= 609.25  end if
					if rs("memno")=4804 and sldate="01/09/2019"  then	divd(i)= 609.25  end if
					if rs("memno")=4804 and sldate="01/10/2019"  then	divd(i)= 609.25  end if
	...
	<%if ttlamt > 100 then %>
		<%if mstatus ="M" or mstatus="T" then %>
			<font face="arial, helvetica, sans-serif" size="3" ><b>　　　　　 - (2) 閣下於本年度股息為<%=formatnumber(ttlamt,2)%>元，以支票支付。</b></font><br>     
		<%else%>
			<font face="arial, helvetica, sans-serif" size="3" ><b>　　　　　 - (2) 閣下於本年度股息為<%=formatnumber(ttlamt,2)%>元，於<%=pyear%>年<%=int(mon)%>月<%=nday%>日自動轉賬入閣下銀行戶口。</b></font><br>
		<%end if%>
	<%else%>
