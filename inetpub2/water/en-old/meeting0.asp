...
if request.form("action") <> "" then
	conn.begintrans
	conn.execute("delete from meetingNotes0 where rpId="&loan)
	if request("thisApp")<>"" then
		A = split(request("thisApp"),",",-1,1)
		if isarray(A) then
			if (ubound(A) >= 0) then
				for i = 0 to ubound(A)
					conn.execute("insert into meetingNotes0 (rpId,appId) values ("&loan&","&A(i)&")")
				next
			end if
		end if
	end if
...
<%
set rs=conn.execute("select a.* from loanApp a,meetingNotes0 b where a.uid=b.appId and b.rpId="&loan&" order by a.uid")
do while not rs.eof
...
<%
set rs=conn.execute("select * from loanApp where (firstApproval='Approved' or secondApproval='Approved') and deleted=0 and uid not in (select appId from meetingNotes0) order by uid")
do while not rs.eof
	response.write "<option value="&rs("uid")&">"&rs("memNo")&" - "&rs("memName")&" - "&formatnumber(rs("loanAmt"),2)
	rs.movenext
loop
