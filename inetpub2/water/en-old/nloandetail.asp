...
if request.form("Search")<>""   then
	...
	if uid <> "" then
		...
		install= installment
                ...
                if oldlnnum<>"" then
                   lnflag ="Y"
                   set rs = conn.execute("select bal from loanrec where repaystat='N' ")
                   if not rs.eof then
                      if loantyoe ="N" then
                          bal =bal +  rs(0)
                      end if
         ...
         if id <> "" then
          set rs = conn.execute("select * from loanapp where DELETED = 0 AND memno='"&memno&"' and (firstapproval='Approved' or secondapproval='Approved') order by memno , appdate desc")    
          if not rs.eof then
             		For Each Field in rs.fields
...
if request.form("action") <> "" then
	...
	'	xmon = right("0"&month(date()),2)
        '       chkdate=xyear&xmon 
                xyear=right(lndate,4)
		xmon = mid(lndate,4,2)

                chkdate=xyear&xmon 
		sql1 = "select max(lnnum) from loanrec  where left(lnnum,6)='"&chkdate&"'  "
		...
		rs1.update
                ...
                conn.execute("insert into loan ( memno,lnnum,code,ldate,amount) values ( '"&memno&"','"&lnnum&"','D9','"&xlndate&"',"&bal&") ")  
...
function calculation(){
	...
	if (formObj.lndate.value!=""){
	   ...
	   DD=parseInt(ssdate.substr(0,2))
           ...
           olnbal =parseFloat(formObj.bal.value)
           lnamt =parseFloat(formObj.appamt.value)
	   chqamt = parseFloat(formObj.chequeamt.value)
...
<center>
		...
		<td width="700" valign="top">
			...
			<tr>
	               <td width=30></td>	
			<td class="b12" align="left">取票金額期</td>
			<td width=50></td>
			<td><input type="text" name="chequeamt" value="<%=chequeamt%>" size="10" maxlength="10"<%if id<>"" then response.write " onfocus=""form1.lndate.focus();""" end if%>></td>		
			</tr>
			<tr>
  		        <td width=30></td>
