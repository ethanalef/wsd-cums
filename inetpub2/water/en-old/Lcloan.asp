...
if request("del")<>"" then
	...
	sql1 = "select * from loan where code like 'E%' and lnnum="&lnnum
	Set rs1 = Server.CreateObject("ADODB.Recordset")
	rs1.open sql1, conn         
        if rs1.eof then
		conn.execute("delete loanrec  where lnnum="&lnnum)
                conn,execute("delete loan where lnnum="&lnnum)
		lnnum =""
        else
...
sql = "select   a.memno,a.lnnum,max(a.lndate),a.appamt,b.memname,b.memhkid  from loanrec a,memmaster b where  a.repaystat='N' and a.memno=b.memno " & sql_filter & "   group by a.memno,a.lnnum ,a.appamt,b.memname,b.memhkid  order  by a.memno "
...
<div><center><font size="3">循環貸款</font></center></div>
...
%>
	<td><a href="lcloandetail.asp?lnnum=<%=idx%><%=rs(1)%><%=xlndate%>"><font size="2"><%=rs(1)%></font></a></td>
...
<%if session("cpageno")>1 then%>
    <a href="lcloan.asp?upage=upage<font size="2">上一頁</font></a>
<%end if%>
<%if session("cpageno")< pagecount then%>
<a href="lcloan.asp?npage=npage<font size="2">下一頁</font></a>
<%end if %>
<%end if%>
</center>
</body>
