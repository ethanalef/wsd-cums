	...
	Next
		...
		if not rs.eof then
			...
			if Field.name="memBday" or Field.name="appointdate" or Field.name="memDate" or Field.name="Wdate" then
				...
				Execute(TheString)
			Next
            if userlevel = 5 or userelevel=3 or userlevel=4  then
		...
		rs("memHKID") = memHKID
		  if memGender="男" then 
		   rs("memGender") = "M" 
                ...
                if userlevel = 3 or userlevel =5 or userlevel = 4  then
                rs("bnk") = bnk
                rs("bch") = bch
                rs("bacct") = bacct
...
if userlevel = 3 or userlevel= 5 then
   working = ""
else
   working = "readonly"
end if
...
<tr >
     ...
     <td align="right">
     <% if id <>"" then %>
     <%if session("userLevel")=3 or session("userLevel")=5 or session("userLevel")=6 then%>
        ...
        <td width="1%" valign="top">
 		...
 		<table border="0" cellspacing="0" cellpadding="0" >
                <tr>
                    ...
                    <td width="10"></td>
                    <td><input type="text" name="memName" value="<%=memName%>" <%=working%> size="35" ></td>
                    <td><font size="2" >中文姓名</formt></td>
                    <td width="10"></td>
                    <td><input type="text" name="memcName" value="<%=memcName%>" <%=working%> size="10" ></td>
               </tr>
               <tr>
                    <td><font size="2" >身份證號碼</formt></td>
                    <td width="10"></td>
                    <td><input type="text" name="memHKID" value="<%=memHKID%>" size="20" <%=working%> onblur="if(!formatHKID(this)){this.value=''}"></td> 
			...
			<option<%if memGender="F" then response.write " selected" end if%>>女</option>
                ...
                <td width="10"></td>
                <td><input type="text" name="memBday" value="<%=memBday%>" size="10" maxlength="10" <%=working%> onblur="if(!formatDate(this)){this.value=''};callage();">(dd/mm/yyyy)</td>
...
%>                  
		...
		<td><font size="2" >銀行</formt></td>
                <td width=48></td>	
                <%if userlevel=3 or userlevel= 5 or userlevel=6 then%>	
                     <td><input type="text" name="bnk" value="<%=bnk%>" <%=working%> <%=working%> size="3"  onchange="BankChange()"></td>
                <%else%>
                     <td><%=xbnk%>
                 <%end if%> 
	
		<td width=5></td>
		<td><input type="text" name="bnkName" value="<%=bnkName%>" <%=working%> size="60"<%if memHKID<>"" then response.write " onfocus=""form1.bch.focus();""" end if%>></td> 
                </tr>  
                </table>
	        <table border="0" cellspacing="0" cellpadding="0" >
       	 	<tr>         
		...
		<td><input type="text" name="bch" value="<%=bch%>" <%=working%> size="3" ></td>
            
		<td width=10></td>
		<td><font size="2" >銀行帳號</formt></td>
		<td width=10></td>
               
		<td><input type="text" name="bacct" value="<%=bacct%>" <%=working%> size="15" ></td>
           
		<td width=10></td>
		<td><font size="2" >轉帳上限</formt></td>
		<td width=1></td>
              
		    <td><input name="bnklmt" value="<%=bnklmt%>" <%=working%> size="10" ></td>                                 
		...
		<td width=5></td>
		     ...
		     <td><input type="text" name="monthsave" value="<%=monthsave%>" <%=working%> size="10" maxlength="10"></td>	
             
		<td width=5></td>
		<td><font size="2" >每月儲蓄(庫房)</formt></td>
		<td width=5></td>
              
		<td><input type="text" name="monthssave" value="<%=monthssave%>" <%=working%> size="10" maxlength="10"></td>	
           
		<td width=5></td>
		<td><font size="2" >庫房過數</formt></td>
		<td width=5></td>
              
		<td><input type="text" name="tpayamt" value="<%=tpayamt%>" <%=working%> size="10" maxlength="10"></td>
