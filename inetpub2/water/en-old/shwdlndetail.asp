	...
	if msg="" then
		...
		conn.begintrans                
                xcleardate = right(cleardate,4)&"/"&mid(cleardate,4,2)&"/"&left(cleardate,2)
                  ...
                  conn.execute("update loanrec set cleardate='"&xcleardate&"'  where  lnnum='"&lnnum&"' and bal=0  ")
		  ...
		  conn.execute("insert into loan (memno,lnnum,code,ldate,amount) values ('"&memno&"','"&lnnum&"','E0','"&xcleardate&"',"&cashamt&")  ")   
                  ...
                  if  pamt > 0  then  
                       conn.execute("update loan set pdate='"&xcleardate&"' where lnnum='"&lnnum&"' and pflag=0 and code='ME'")  
                  ...
                  if  pint > 0 then
                        conn.execute("update loan set  pdate='"&xcleardate&"'  where lnnum='"&lnnum&"' and pflag=0 and code='MF' ")
                       conn.execute("update loan set pflag=0   where lnnum='"&lnnum&"' and pflag=1 and code='MF' ")
  
                  end if 
                 end if    
		  sql1 = " select * from share where memno = "& memno                  
                  ...
                  rs1.close  
                  if sbal =  0 then
                      conn.execute("update memmaster set wdate = '"&xcleardte&"'  where memno='"&memno&"' " )
                  end if
		conn.committrans
		msg = "紀錄已更新"
             end if
...
<center>
	...
	<tr>
               <td width=30></td>
		<td class="b12" align="left">清數本金結餘</td>
		<td width=24></td>
