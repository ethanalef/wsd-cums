...
if xlnnum <> "" then
SQl = "select  *,convert(char(10),ldate,102) as Expr1  from loan  where memno='"&id&"'  order by memno,ldate,right(code,1),left(code,1) "
Set rs = Server.CreateObject("ADODB.Recordset")	
rs.open sql, conn,2  
...
do while not rs.eof
  ...
  xdate = right("0"&day(rs("ldate")),2)&"/"&right("0"&month(rs("ldate")),2)&"/"&year(rs("ldate"))
      ...
      if (rs("code")="F3"  or rs("code")="F1" or rs("code")="F2" or rs("code")="F7" or rs("code")="F0"  )   then
